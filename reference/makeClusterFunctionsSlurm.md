...
`sbatch` command. Jobs are killed using the `scancel` command and the
list of running jobs is retrieved using `squeue`. The user must have the
...
Note that you might have to specify the cluster name here if you do not
want to use the default, otherwise the commands for listing and killing
jobs will not work.
