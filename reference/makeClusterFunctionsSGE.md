...
Cluster functions for Univa Grid Engine / Oracle Grid Engine / Sun Grid
Engine (<https://altair.com/hpc-cloud-applications/>).

Job files are created based on the brew template `template`. This file
is processed with brew and then submitted to the queue using the `qsub`
command. Jobs are killed using the `qdel` command and the list of
running jobs is retrieved using `qselect`. The user must have the
appropriate privileges to submit, delete and list jobs on the cluster
