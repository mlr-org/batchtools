...
They are created with the constructor [makeClusterFunctions](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctions) which defines how jobs are submitted on your system.
...
* Interactive Cluster Functions (default): [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsInteractive), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsInteractive.R)
* Multicore Cluster Functions: [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsMulticore), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsMulticore.R)
* Socket Cluster Functions: [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsSocket), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsSocket.R)
* Makeshift SSH cluster: [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsSSH), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsSSH.R)
* Docker Swarm: [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsDocker), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsDocker.R)
* IBM Spectrum Load Sharing Facility (LSF): [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsLSF), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsLSF.R)
* OpenLava: [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsOpenLava), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsOpenLava.R)
* Univa Grid Engine / Oracle Grid Engine (OGE) / Sun Grid Engine (SGE): [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsSGE), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsSGE.R)
* Slurm: [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsSlurm), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsSlurm.R)
* TORQUE/OpenPBS: [docs](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsTORQUE), [implementation](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsTORQUE.R)


To use the package with the socket cluster functions, you would call the respective constructor [makeClusterFunctionsSocket()](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctionsSocket):
```{r, message=FALSE}
reg = makeRegistry(NA)
reg$cluster.functions = makeClusterFunctionsSocket(2)
```
To make this selection permanent for this registry, save the Registry with [saveRegistry()](https://batchtools.mlr-org.com/batchtools/reference/makeRegistry).
...
Note that all variables defined in a [JobCollection](https://batchtools.mlr-org.com/batchtools/reference/JobCollection) can be used inside the template.
If you need to pass extra variables, you can set them via the argument `resources` of [`submitJobs()`](https://batchtools.mlr-org.com/batchtools/reference/submitJobs).

If the flexibility which comes with templating is not sufficient, you can still construct a custom cluster function implementation yourself using the provided [constructor](https://batchtools.mlr-org.com/batchtools/reference/makeClusterFunctions).


## Configuration File

The configuration file can be used to set system specific options.
Its default location depends on the operating system (see [Registry](https://batchtools.mlr-org.com/batchtools/reference/makeRegistry)), but for the first time setup you can put one in the current working directory (as reported by `getwd()`).
In order to set the cluster function implementation, you would generate a file with the following content:
```{r,eval = FALSE}
cluster.functions = makeClusterFunctionsInteractive()
```
The configuration file is parsed whenever you create or load a [Registry](https://batchtools.mlr-org.com/batchtools/reference/makeRegistry).
It is sourced inside of your registry which has the advantage that you can (a) access all of the parameters which are passed to [makeRegistry](https://batchtools.mlr-org.com/batchtools/reference/makeRegistry) and (b) you can also directly change them.
...
```
See the documentation on [Registry](https://batchtools.mlr-org.com/batchtools/reference/makeRegistry) for a more complete list of supported configuration options.
...
* Nodes do not have to access the registry.
  [submitJobs()](https://batchtools.mlr-org.com/batchtools/reference/submitJobs) stores a temporary object of type [JobCollection](https://batchtools.mlr-org.com/batchtools/reference/JobCollection) on the file system which holds all the information necessary to execute a chunk of jobs via [doJobCollection()](https://batchtools.mlr-org.com/batchtools/reference/doJobCollection) on the node.
...
* Most functions now return a [data.table](https://cran.r-project.org/package=data.table) which is keyed with the `job.id`.
  This way, return values can be joined together easily and efficient (see this [help page](https://batchtools.mlr-org.com/batchtools/reference/JoinTables) for some examples).
...
* The function `makeDesign` has been removed.
  Parameters can be defined by just passing a `data.frame` or `data.table` to [addExperiments](https://batchtools.mlr-org.com/batchtools/reference/addExperiments).
...
* All information stored in the object [`JobCollection`](https://batchtools.mlr-org.com/batchtools/reference/JobCollection) can be accessed while brewing the template.
* Extra variables may be passed via the argument `resoures` of [submitJobs](https://batchtools.mlr-org.com/batchtools/reference/submitJobs).
...
* Some resources like the number of CPUs are now optionally passed to [parallelMap](https://cran.r-project.org/package=parallelMap).
  ...
  See [submitJobs()](https://batchtools.mlr-org.com/batchtools/reference/submitJobs) for an example.
* `ClusterFunctions` are now more flexible in general as they can define hook functions which will be called at certain events.
  [ClusterFunctionsDocker](https://github.com/mlr-org/batchtools/blob/master/R/clusterFunctionsDocker.R) is an example use case which implements a housekeeping routine.
...
* More new features are covered in the [NEWS](https://batchtools.mlr-org.com/batchtools/news/index.html).
...
| -------------            | :-------------:                                   |
| `addRegistryPackages`    | Set `reg$packages` or `reg$namespaces`, call [saveRegistry()](https://batchtools.mlr-org.com/batchtools/reference/saveRegistry) |
| `addRegistrySourceDirs`  | -                                                                                                                       |
| `addRegistrySourceFiles` | Set `reg$source`, call [saveRegistry()](https://batchtools.mlr-org.com/batchtools/reference/saveRegistry)                       |
| `batchExpandGrid`        | [batchMap](https://batchtools.mlr-org.com/batchtools/reference/batchMap): `batchMap(..., args = CJ(x = 1:3, y = 1:10))`         |
| `batchMapQuick`          | [btmapply](https://batchtools.mlr-org.com/batchtools/reference/btlapply)                                                        |
| `batchReduceResults`     | -                                                                                                                       |
| `batchUnexport`          | [batchExport](https://batchtools.mlr-org.com/batchtools/reference/batchExport)                                                  |
| `filterResults`          | -                                                                                                                       |
| `getJobIds`              | [findJobs](https://batchtools.mlr-org.com/batchtools/reference/findJobs)                                                        |
| `getJobInfo`             | [getJobStatus](https://batchtools.mlr-org.com/batchtools/reference/getJobTable)                                                 |
| `getJob`                 | [makeJob](https://batchtools.mlr-org.com/batchtools/reference/JobExperiment)                                                    |
| `getJobParamDf`          | [getJobPars](https://batchtools.mlr-org.com/batchtools/reference/getJobTable)                                                   |
| `loadResults`            | [reduceResultsList](https://batchtools.mlr-org.com/batchtools/reference/reduceResultsList)                                      |
| `reduceResultsDataFrame` | [reduceResultsDataTable](https://batchtools.mlr-org.com/batchtools/reference/reduceResultsList)                                 |
| `reduceResultsMatrix`    | [reduceResultsList](https://batchtools.mlr-org.com/batchtools/reference/reduceResultsList) + `do.call(rbind, res)`              |
| `reduceResultsVector`    | [reduceResultsDataTable](https://batchtools.mlr-org.com/batchtools/reference/reduceResultsList)                                 |
| `setJobFunction`         | -                                                                                                                       |
| `setJobNames`            | -                                                                                                                       |
| `showStatus`             | [getStatus](https://batchtools.mlr-org.com/batchtools/reference/getStatus)                                                      |
...
There are two different types of registry objects: First, a regular [`Registry`](https://batchtools.mlr-org.com/batchtools/reference/makeRegistry) which we will use in this example.
Second, an [`ExperimentRegistry`](https://batchtools.mlr-org.com/batchtools/reference/makeExperimentRegistry) which provides an alternative way to define computational jobs and thereby is tailored for a broad range of large scale computer experiments.
...
To get the current default registry, [`getDefaultRegistry`](https://batchtools.mlr-org.com/batchtools/reference/getDefaultRegistry) can be used.
To switch to another registry, use [`setDefaultRegistry()`](https://batchtools.mlr-org.com/batchtools/reference/getDefaultRegistry).
...
We use [`batchMap()`](https://batchtools.mlr-org.com/batchtools/reference/batchMap) to define the jobs (note that this does not yet start the calculation):
```{r}
...
An overview over the jobs and their IDs can be retrieved with [`getJobTable()`](https://batchtools.mlr-org.com/batchtools/reference/getJobTable) which returns a data.frame with all relevant information:
```{r}
names(getJobTable())
```
Note that a unique job ID is assigned to each job.
These IDs can be used to restrict operations to subsets of jobs.
To actually start the calculation, call [`submitJobs()`](https://batchtools.mlr-org.com/batchtools/reference/submitJobs).
...
The progress of the submitted jobs can be checked with [`getStatus()`](https://batchtools.mlr-org.com/batchtools/reference/getStatus).
```{r}
getStatus()
```
...
This can be done in a simple fashion by using either [`loadResult()`](https://batchtools.mlr-org.com/batchtools/reference/loadResult), which returns a single result exactly in the form it was calculated during mapping, or by using [`reduceResults()`](https://batchtools.mlr-org.com/batchtools/reference/reduceResults), which is a version of `Reduce()` from the base package for registry objects.
```{r}
waitForJobs()
...
If you are absolutely sure that your function works, you can take a shortcut and use *batchtools* in an `lapply` fashion using [`btlapply()`](https://batchtools.mlr-org.com/batchtools/reference/btlapply).
This function creates a temporary registry (but you may also pass one yourself), calls [`batchMap()`](https://batchtools.mlr-org.com/batchtools/reference/reduceResultsList), wait for the jobs to terminate with [`waitForJobs()`](https://batchtools.mlr-org.com/batchtools/reference/waitForJobs) and then uses [`reduceResultsList()`](https://batchtools.mlr-org.com/batchtools/reference/reduceResultsList) to return the results.
...
We use an [`ExperimentRegistry`](https://batchtools.mlr-org.com/batchtools/reference/makeExperimentRegistry) where the job definition is split into creating problems and algorithms.
...
Therefore the function [`addProblem()`](https://batchtools.mlr-org.com/batchtools/reference/addProblem) accepts static parts in its `data` argument, which is passed to the argument `fun` which generates a (possibly stochastic) problem instance.
...
The argument `job` is an object of type [`Job`](https://batchtools.mlr-org.com/batchtools/reference/JobExperiment) which holds additional information about the job.
...
```
[`addProblem()`](https://batchtools.mlr-org.com/batchtools/reference/addProblem) files the problem to the file system and the problem gets recorded in the registry.
```{r}
...
When using [`addAlgorithm()`](https://batchtools.mlr-org.com/batchtools/reference/addAlgorithm), an identifier as well as the algorithm to apply to are required arguments.
...
## Creating jobs
[`addExperiments()`](https://batchtools.mlr-org.com/batchtools/reference/addExperiments) is used to parametrize the jobs and thereby define computational jobs.
To do so, you have to pass named lists of parameters to [`addExperiments()`](https://batchtools.mlr-org.com/batchtools/reference/addExperiments).
...
When the problem design and the algorithm design are combined in [`addExperiments()`](https://batchtools.mlr-org.com/batchtools/reference/addExperiments), each combination of the parameter sets of the two designs defines a distinct job.
...
The function [`summarizeExperiments()`](https://batchtools.mlr-org.com/batchtools/reference/summarizeExperiments) returns a table which gives a quick overview over all defined experiments.
...
For all these use cases, [`findExperiments()`](https://batchtools.mlr-org.com/batchtools/reference/findJobs) can be employed to conveniently select a particular subset of jobs.
...
To illustrate [`findExperiments()`](https://batchtools.mlr-org.com/batchtools/reference/findJobs), we will select two experiments, one with a support vector machine and the other with a random forest and the parameter `ntree = 1000`.
...
To submit the jobs, we call [`submitJobs()`](https://batchtools.mlr-org.com/batchtools/reference/submitJobs) and wait for all jobs to terminate using [`waitForJobs()`](https://batchtools.mlr-org.com/batchtools/reference/waitForJobs).
```{r}
submitJobs()
waitForJobs()
```
After jobs are finished, the results can be collected with [`reduceResultsDataTable()`](https://batchtools.mlr-org.com/batchtools/reference/reduceResultsList) where we directly extract the mean misclassification error:
```{r}
...
Next, we merge the results table with the table of job parameters using one of the [join helpers](https://batchtools.mlr-org.com/batchtools/reference/JoinTables) provided by `batchtools` (here, we use an inner join):
```{r}
...
Therefore, before you submit anything you should use [`testJob()`](https://batchtools.mlr-org.com/batchtools/reference/testJob) to catch errors that are easy to spot because they are raised in many or all jobs.
...
waitForJobs()
```
After you have submitted jobs and suspect that something is going wrong, the first thing to do is to run [`getStatus()`](https://batchtools.mlr-org.com/batchtools/reference/getStatus) to display a summary of the current state of the system.
```{r}
getStatus()
```
The status message shows that two of the jobs could not be executed successfully.
To get the IDs of all jobs that failed due to an error we can use [`findErrors()`](https://batchtools.mlr-org.com/batchtools/reference/findJobs) and to retrieve the actual error message, we can use [`getErrorMessages()`](https://batchtools.mlr-org.com/batchtools/reference/getErrorMessages).
```{r}
findErrors()
getErrorMessages()
```
If we want to peek into the R log file of a job to see more context for the error we can use [`showLog()`](https://batchtools.mlr-org.com/batchtools/reference/showLog) which opens a pager or use [`getLog()`](https://batchtools.mlr-org.com/batchtools/reference/showLog) to get the log as character vector:
```{r}
tail(getLog(id = 9))
```
...
1. Create a Registry with [`makeRegistry()`](https://batchtools.mlr-org.com/batchtools/reference/makeRegistry) (or [`makeExperimentRegistry()`](https://batchtools.mlr-org.com/batchtools/reference/makeExperimentRegistry)) or load an existing from the file system with [`loadRegistry()`](https://batchtools.mlr-org.com/batchtools/reference/loadRegistry).
2. Define computational jobs with [`batchMap()`](https://batchtools.mlr-org.com/batchtools/reference/batchMap) or [`batchReduce()`](https://batchtools.mlr-org.com/batchtools/reference/batchReduce) if you used [`makeRegistry()`](https://batchtools.mlr-org.com/batchtools/reference/makeRegistry) or define with [`addAlgorithm()`](https://batchtools.mlr-org.com/batchtools/reference/addAlgorithm), [`addProblem()`](https://batchtools.mlr-org.com/batchtools/reference/addProblem) and [`addExperiments()`](https://batchtools.mlr-org.com/batchtools/reference/addExperiments) if you started with [`makeExperimentRegistry()`](https://batchtools.mlr-org.com/batchtools/reference/makeExperimentRegistry).
   It is advised to test some jobs with [`testJob()`](https://batchtools.mlr-org.com/batchtools/reference/testJob) in the interactive session and with `testJob(external = TRUE)` in a separate R process.
   Note that you can add additional jobs if you are using an [`ExperimentRegistry`](https://batchtools.mlr-org.com/batchtools/reference/makeExperimentRegistry).
3. If required, query the data base for job ids depending on their status, parameters or tags (see [`findJobs()`](https://batchtools.mlr-org.com/batchtools/reference/findJobs)).
   The returned tables can easily be combined in a set-like fashion with data base verbs: union ([`ojoin()`](https://batchtools.mlr-org.com/batchtools/reference/JoinTables) for outer join), intersect ([`ijoin()`](https://batchtools.mlr-org.com/batchtools/reference/JoinTables) for inner join), difference ([`ajoin()`](https://batchtools.mlr-org.com/batchtools/reference/JoinTables) for anti join).
4. Submit jobs with [`submitJobs()`](https://batchtools.mlr-org.com/batchtools/reference/submitJobs). You can specify job resources here.
   If you have thousands of fast terminating jobs, you want to [`chunk()`](https://batchtools.mlr-org.com/batchtools/reference/chunk) them first.
   If some jobs already terminated, you can estimate the runtimes with [`estimateRuntimes()`](https://batchtools.mlr-org.com/batchtools/reference/estimateRuntimes) and chunk jobs into heterogeneous groups with [`lpt()`](https://batchtools.mlr-org.com/batchtools/reference/chunk) and [`binpack()`](https://batchtools.mlr-org.com/batchtools/reference/chunk).
5. Monitor jobs. [`getStatus()`](https://batchtools.mlr-org.com/batchtools/reference/getStatus) gives a summarizing overview.
   Use [`showLog()`](https://batchtools.mlr-org.com/batchtools/reference/showLog) and [`grepLogs()`](https://batchtools.mlr-org.com/batchtools/reference/grepLogs) to investigate log file.
   Run jobs in the currently running session with [`testJob()`](https://batchtools.mlr-org.com/batchtools/reference/testJob) to get a `traceback()`.
6. Collect (partial) results. [`loadResult()`](https://batchtools.mlr-org.com/batchtools/reference/loadResult) retrieves a single result from the file system.
   [`reduceResults()`](https://batchtools.mlr-org.com/batchtools/reference/reduceResults) mimics `Reduce()` and allows to apply a function to many files in an iterative fashion.
   [`reduceResultsList()`](https://batchtools.mlr-org.com/batchtools/reference/reduceResultsList) and [`reduceResultsDataTable()`](https://batchtools.mlr-org.com/batchtools/reference/reduceResultsList) collect results into a `list` or `data.table`, respectively.
...
This can be done by prototyping locally ([`testJob()`](https://batchtools.mlr-org.com/batchtools/reference/testJob) or submit subsets via [`submitJobs()`](https://batchtools.mlr-org.com/batchtools/reference/submitJobs)).
...
After loading the Registry, it is advised to test some jobs again with [`testJob()`](https://batchtools.mlr-org.com/batchtools/reference/testJob) before submitting all of them with `submitJobs(resources = list())` (remember you now need to set resources!).
After some jobs are finished, the `file.dir` can be copied back (do not merge with the previous directory!) and loaded again with [`loadRegistry()`](https://batchtools.mlr-org.com/batchtools/reference/loadRegistry).
...
5. Partial results can be inspected both on the remote system and on the local system.
   ...
   Load the registry locally with [`loadRegistry()`](https://batchtools.mlr-org.com/batchtools/reference/loadRegistry) and collect results. Do not copy back and forth.
